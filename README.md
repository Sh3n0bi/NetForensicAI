...
- **Network protocol analysis** — pcap parsing extracts TCP/UDP/ICMP flows over **both IPv4 and IPv6**, DNS queries *and responses* (so you know what a name actually resolved to), HTTP requests and status codes, and TLS SNI hostnames — so domains and URLs become first-class entities you can pivot on even in encrypted traffic
...
- Ingestion, correlation and detections all stream: events go parser → database in batches (`INGEST_BATCH_SIZE`) and analyses read them back through a cursor, so peak memory tracks working set rather than capture size. Correlation still holds one entity-link map proportional to the case, so memory is much reduced but not strictly constant
